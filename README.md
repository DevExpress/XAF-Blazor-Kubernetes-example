...
```
docker run --publish ostashev/xafloadtesting:latest .
```
...
```
docker exec <your_container_id> dotnet LoadTestingApp.Blazor.Server.dll --updateDatabase --forceUpdate --silent
```
...
```
docker run --network="host" -e CONNECTION_STRING=MySqlConnectionString ostashev/xafloadtesting:latest .
```
